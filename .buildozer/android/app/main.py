...
class MenuButton(Button):
	...
	def pressed(self):
		# Coloring is done in the .kv file
		Clock.schedule_once(self.unpressed, 1)
	
	def unpressed(self, _delta):
...
class StatsScreen(Screen):
	"""
	The screen responsilbe for displaying the highscore and other important stats.
	"""
	Highscore = NumericProperty(0)
	
	def __init__(self, **kwargs):
		super().__init__(**kwargs)
		self.scores_to_update = []
		Clock.schedule_interval(self.update_db_clock, 60)
		Clock.schedule_once(self.get_highscore, 0)
	...
	def update_db_clock(self, _delta):
		self.update_db()
	
	def update_db(self):
		print("Updating Database...")
		conn = self.manager.db_connection
		for score in self.scores_to_update:
			conn.execute("INSERT INTO games VALUES(?, ?)", (score[0], score[1]))
		conn.commit()
		self.scores_to_update.clear()
...
class GameLayout(GridLayout):
	"""
	The layout for the GameScreen composed from the TopBar that displays the Score and Lives and
	the Game itself
	"""
...
class TopBar(AnchorLayout):
	""" The ribbon that displays the Score and Lives"""
	pass
			...
			for ball in self.children:
				...
				if ball.y <= 0:
					...
					if self.lives == 0:
						...
						self.parent.parent.manager.get_screen("stats").scores_to_update.append(
							(self.time_started, self.score))
...
class ShopScreen(Screen):
	...
	# @staticmethod
	# def convert_string_to_bool(value: str):
	# 	if value == "True":
	# 		return True
	# 	else:
	# 		return False
...
class ShootTheBallsApp(App):
	...
	def on_stop(self):
		if not self.stopped:
			self.stopped = True
			stats_screen = self.app_manager.get_screen("stats")
			shop_screen = self.app_manager.get_screen("shop")
			shop_screen.save_skins_data()
			shop_screen.save_coins()
			# shop_screen.skins_file_store.clear()
			try:
				self.app_manager.db_connection.cursor()  # check if the connection still exists
				stats_screen.update_db()
				self.app_manager.db_connection.close()
